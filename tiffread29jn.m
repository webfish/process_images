function [stack, meanstack] = tiffread29jn(filename, indices)
...
%
% Function modified to be produce with dip_image data format output. JN.
...
while  TIF.img_pos ~= 0 
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        if read_img
            TIF.StripCnt = 1;
            if nargout == 2 && ~exist('meanstack', 'var')
                meanstack = zeros(IMG.height, IMG.width, TIF.SamplesPerPixel);
            end
            %read the image channels
            for c = 1:TIF.SamplesPerPixel
                ...
                if nargout == 2
                    if TIF.SamplesPerPixel > 1
                        meanstack(:, :, c) = meanstack(:, :, c) + double(IMG.data{c});
                    else
                        meanstack = meanstack + double(IMG.data{c});
                    end
                end
            end
